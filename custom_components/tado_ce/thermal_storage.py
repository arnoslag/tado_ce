...
class ThermalStorage:
    ...
    async def async_load(self) -> int:
        ...
        # Try new format first (use aiofiles.os.path.exists to avoid blocking)
    ...
    async def _migrate_from_old_formats(self) -> int:
        ...
        # Check file existence using async I/O
        sc_exists = await aiofiles.os.path.exists(smart_comfort_path)
        sc_legacy_exists = await aiofiles.os.path.exists(legacy_smart_comfort)
        hc_exists = await aiofiles.os.path.exists(heating_cycle_path)
        hc_legacy_exists = await aiofiles.os.path.exists(legacy_heating_cycle)

        # Migrate smart_comfort_cache
        sc_path = smart_comfort_path if sc_exists else (
            legacy_smart_comfort if sc_legacy_exists else None
        )
        if sc_path:
        ...
        hc_path = heating_cycle_path if hc_exists else (
            legacy_heating_cycle if hc_legacy_exists else None
        )
        if hc_path:
    ...
    async def _backup_corrupted_file(self) -> None:
        ...
        if await aiofiles.os.path.exists(self._storage_path):
