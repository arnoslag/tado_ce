"""Tado CE unified thermal analytics storage — smart comfort + heating cycle data."""
...
class ThermalStorage:
    ...
    def __init__(self, hass: HomeAssistant, home_id: str, history_days: int = 30) -> None:
        """Initialize storage."""
        self._hass = hass
    ...
    def zones(self) -> dict[str, Any]:
        """Get zones data."""
        return self._data.get("zones", {})  # type: ignore[no-any-return]
    ...
    async def _load_new_format(self) -> int:
        ...
        try:
            ...
            total_readings = sum(len(z.get("temperature_history", [])) for z in self._data.get("zones", {}).values())
            ...
            _LOGGER.exception("Corrupted thermal analytics cache")
            await self._backup_corrupted_file()
            return 0
        except Exception:
            _LOGGER.exception("Failed to load thermal analytics cache")
            return 0
    ...
    async def _migrate_from_old_formats(self) -> int:
        ...
        legacy_smart_comfort = Path(
            self._hass.config.path(
                ".storage/tado_ce/smart_comfort_cache.json",
            ),
        )
        legacy_heating_cycle = Path(
            self._hass.config.path(
                ".storage/tado_ce/heating_cycle_history.json",
            ),
        )

        # Check file existence using executor
        sc_exists = await self._hass.async_add_executor_job(smart_comfort_path.exists)
        sc_legacy_exists = await self._hass.async_add_executor_job(legacy_smart_comfort.exists)
        hc_exists = await self._hass.async_add_executor_job(heating_cycle_path.exists)
        hc_legacy_exists = await self._hass.async_add_executor_job(legacy_heating_cycle.exists)

        # Migrate smart_comfort_cache
        sc_path = smart_comfort_path if sc_exists else (legacy_smart_comfort if sc_legacy_exists else None)
        if sc_path:
        ...
        hc_path = heating_cycle_path if hc_exists else (legacy_heating_cycle if hc_legacy_exists else None)
        if hc_path:
    ...
    async def _migrate_smart_comfort_cache(self, path: Path) -> int:
        ...
        try:
            ...
            _LOGGER.exception("Failed to migrate smart_comfort_cache")
            return 0
    ...
    async def _migrate_heating_cycle_history(self, path: Path) -> int:
        ...
        try:
            ...
            if "version" in old_data and "zones" in old_data:
                ...
                zones_data = {
                    zone_id: {"cycles": cycles} for zone_id, cycles in old_data.items() if isinstance(cycles, list)
                }
            ...
            _LOGGER.exception("Failed to migrate heating_cycle_history")
            return 0
    ...
    async def _backup_corrupted_file(self) -> None:
        ...
        if exists:
            ...
            try:
                ...
                _LOGGER.exception("Failed to backup corrupted file")
    ...
    async def async_save(self) -> bool:
        ...
        try:
            ...
            self._data["saved_at"] = datetime.now(UTC).isoformat()
            ...
            _LOGGER.exception("Failed to save thermal analytics cache")
            return False

    def get_zone_data(self, zone_id: str) -> dict[str, Any] | None:
        """Get data for a specific zone."""
        return self._data["zones"].get(str(zone_id))  # type: ignore[no-any-return]
    ...
    def _cleanup_old_cycles(self, zone_id: str) -> None:
        ...
        cutoff = datetime.now(UTC) - timedelta(days=self._history_days * 2)
    ...
    def get_temperature_history(
        ...
        if window_days is not None:
            ...
            readings = [r for r in readings if datetime.fromisoformat(r["timestamp"]) >= cutoff]

        return readings  # type: ignore[no-any-return]
