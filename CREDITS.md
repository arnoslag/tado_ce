...
- **[@apilone](https://github.com/apilone)** — Laid out the holiday-automation problem in [#256](https://github.com/hiall-fyi/tado_ce/issues/256) with the use case fully worked through: Tado has no holiday support, so the workaround is an automation that raises the target on a bridge day, but Smart Valve Control reads that write as a manual slider grab and backs off, leaving the room cold. He also sketched the fix that turned out to be the right one, a service that updates the desired target in place and skips the manual-override detection. That's essentially what shipped as `set_schedule_temperature`.
- **[@beltra](https://github.com/beltra)** — Reported presence mode reverting to Home after a restart ([#302](https://github.com/hiall-fyi/tado_ce/issues/302)) with the detail that pinned it: the logbook showed the change had no user or automation behind it, so the integration itself was setting it on startup, while the geofence trackers read correctly at the same moment. He also chased down that the Tado-side flip he saw was one of his own automations, not Tado CE, which ruled out a phantom cloud write and narrowed it straight to the local restore. The cache write turned out never to reach disk, so an Away set with Home State Sync off didn't survive a restart.
- **[@davidjirovec](https://github.com/davidjirovec)** — Filed [#303](https://github.com/hiall-fyi/tado_ce/issues/303) as the clearest statement yet of a need that had been building since [#276](https://github.com/hiall-fyi/tado_ce/issues/276): on a 20,000-call plan, the 5-minute floor on presence and mobile locations is an artificial limit, and the latency it adds is real for automations. @churchofnoise raised the same point in #276 and #289 when the floor was first set; the decision then was to hold on configurable intervals until the quota and cadence work was solid enough to expose them safely. #303 was the point where the foundation had settled and the demand was clear enough to act.
- **[@wrowlands3](https://github.com/wrowlands3)** — Followed up on the beta.4 round-robin fix with a log showing the offset fetch burst hadn't actually gone away ([#304](https://github.com/hiall-fyi/tado_ce/issues/304)). The beta.4 change fixed the periodic drift refresh path; the startup full-sync path was still reading every zone's offset on every restart and every hour, gated only on the display toggle rather than on whether any zone had Smart Valve Control active. The fix gates the fetch on SVC being enabled on at least one zone.
...
- **[@apilone](https://github.com/apilone)** — Reported the climate state and temperature flickering out of sync after a change on a hybrid HomeKit + cloud zone ([#296](https://github.com/hiall-fyi/tado_ce/issues/296)), with a full debug log and a step-by-step repro showing exactly which app-side change put HA out of sync and how long it took to settle. The log made the root cause plain: HomeKit and the cloud were both writing the HVAC mode, and since HomeKit can't express "following your schedule", they disagreed on every poll. The mode now comes from the cloud only, which stops the flap; room temperature still follows HomeKit for speed.
- **[@churchofnoise](https://github.com/churchofnoise)** — Asked why the cloud temperature seemed to update more often than the poll interval ([#293](https://github.com/hiall-fyi/tado_ce/issues/293)) and stuck with the back-and-forth as it turned out to be interpolation rather than a faster feed. That thread is what surfaced the need to let each zone choose which temperature reading the dashboard shows, which landed as the new per-zone Temperature source setting.
- **[@bobbinz](https://github.com/bobbinz)** — Asked for a way to tell which physical TRV is which ([#291](https://github.com/hiall-fyi/tado_ce/issues/291)) and tested the `identify_device` service both ways, with and without the full serial from the Tado app. The "even the app serial errors" detail pinned a real bug: the service only knew how to resolve a bridge serial, so it could never find a TRV or thermostat. That's now fixed, and the dig into what the bridge exposes locally turned up a native HomeKit Identify characteristic, so the new per-device button flashes the LED over your LAN at no quota cost.
