...
| ☕☕ | Arnaud L., [@hapklaar](https://github.com/hapklaar), [@jeverley](https://github.com/jeverley), [@jeromewir](https://github.com/jeromewir), Luke R., [@marcovn](https://github.com/marcovn), [@Prodeguerriero](https://github.com/Prodeguerriero) |
...
- **[@apilone](https://github.com/apilone)** — Reported the HomeKit regression against the beta.14 frost-protection fix with a precise screenshot sequence showing the 5°C → 19.5°C loop every few minutes ([#258](https://github.com/hiall-fyi/tado_ce/issues/258)). The specific timing and the stale-cached-target behaviour made it clear the HomeKit bridge was pushing the last heating target back over the OFF state. Same root cause as @apilone's original #253 report, which shipped the 3-minute HomeKit write protection window.
- **[@wrowlands3](https://github.com/wrowlands3)** — Shared a Valve Target graph showing the target temperature oscillating between ~19°C and ~21°C while near the setpoint ([Discussion #219](https://github.com/hiall-fyi/tado_ce/discussions/219)). Confirmed the same HomeKit stale-bridge-target bug from #253 was affecting Valve Target users too, not just Offset Sync — which justified extending the 3-minute write protection window to cloud writes as well.
- **[@Newreader](https://github.com/Newreader)** — Reported that the climate card kept showing the previous heat target (23°C) after `set_hvac_mode: off` while the Tado app correctly showed Frost Protection (5°C) ([#258](https://github.com/hiall-fyi/tado_ce/issues/258)). The Developer Tools state dump and step-by-step reproduction made the OFF-path temperature-update gap trivial to trace on the cloud-only code path.
- **[@simonotter](https://github.com/simonotter)** — Pushed back on the "only writes when the zone is actively heating" wording after the beta.14 schedule gate ([#221](https://github.com/hiall-fyi/tado_ce/issues/221)), raising a realistic overnight-schedule scenario (17°C target, external 16°C, TRV 17.5°C) that sounded like a potential deadlock. Although the gate logic was actually correct, the exchange made it clear the log wording could mislead users — and directly led to the new per-evaluation debug log line in this release so others can verify the controller's decisions without guessing.
- **[@driagi](https://github.com/driagi)** — Reopened the Weather Compensation "Unknown" issue after beta.14's outdoor-grace-period fix didn't fully solve it ([#249](https://github.com/hiall-fyi/tado_ce/issues/249)). Provided the decisive data points: the Status sensor showing `paused`, a clean outdoor source history with no gaps, 8-to-48-hour stuck periods starting consistently after midnight, and finally the key context that he had switched from aggressive ~7-minute polling to Auto night polling (120 min) after HomeKit shipped. Four rounds of targeted evidence triangulated a second, deeper bug — a stale-reading gate that latched the engine into paused whenever the gap between evaluations exceeded 60 min. Fix shipped in beta.15.
