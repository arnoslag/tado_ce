...
- **[@churchofnoise](https://github.com/churchofnoise)** — Surfaced that adaptive polling wasn't taking advantage of the grandfathered 20,000-call legacy quota tier ([#276](https://github.com/hiall-fyi/tado_ce/issues/276)). His report (145 calls used out of a 20,000-call daily allowance, 16 hours into the day) was the concrete data point that forced a re-read of the adaptive polling math: Tado's response headers report the quota as a `limit` field that the integration was already reading, and the math was set up to widen or narrow based on it, but the narrowing direction was capped at a 5 minute floor that had been sized for the 100-call free tier years earlier. The cap was the only thing keeping the 20K tier from getting sub-5-minute polling automatically. His push-back on "just set a custom interval" was the right call: a knob is not the same as adaptive doing its job, and the floor reduction shipped here narrows the math to a 1-minute physics floor (Tado's cloud doesn't update faster than that anyway) so the 20K and 1,000-call tiers naturally land on tighter intervals while the 100-call tier stays unaffected because the math widens well above the floor on that budget.
